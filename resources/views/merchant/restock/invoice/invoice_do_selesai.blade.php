	...
	<body>
		<div class="invoice-box">
			<table>
				...
				@foreach ($deliveryOrderSelesai as $item)
				<tr class="item">
					<td>{{ $item->ProductName }}</td>
					<td class="text-center">{{ $item->Qty }}</td>
					<td class="text-right">{{ Helper::formatCurrency($item->Price, 'Rp ') }}</td>
					<td class="text-right">{{ Helper::formatCurrency($item->Price * $item->Qty, 'Rp ') }}</td>
				</tr>		
				@endforeach
				<tr>
					<td></td>
				...
				@if ($deliveryOrder->Discount != 0)
				<tr class="total">
					<th colspan="3" class="text-right pt">Diskon</th>
					<th colspan="1" class="text-right pt">{{ Helper::formatCurrency($deliveryOrder->Discount, 'Rp ') }}</th>
				</tr>
				@endif
				@if ($deliveryOrder->ServiceCharge != 0)
				<tr class="total">
					<th colspan="3" class="text-right pt">Biaya Layanan</th>
					<th colspan="1" class="text-right pt">{{ Helper::formatCurrency($deliveryOrder->ServiceCharge, 'Rp ') }}</th>
				</tr>
				@endif
				@if ($deliveryOrder->DeliveryFee != 0)
				<tr class="total">
					<th colspan="3" class="text-right pt">Biaya Pengiriman</th>
					<th colspan="1" class="text-right pt">{{ Helper::formatCurrency($deliveryOrder->DeliveryFee, 'Rp ') }}</th>
				</tr>
				@endif
				<tr class="total">
					<th colspan="3" class="text-right pt">Grand Total</th>
					<th colspan="1" class="text-right pt">{{ Helper::formatCurrency($subTotal - $deliveryOrder->Discount + $deliveryOrder->ServiceCharge + $deliveryOrder->DeliveryFee, 'Rp ') }}</th>
				</tr>
			</table>
		</div>
		<script>
